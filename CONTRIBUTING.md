...
- **Read `AGENTS.md` first** (maintenance notes). It lists the traps that already bit this project (the lock is global and not in the per-source sections; `pmset -g cap` ignores `-b/-c`; windows open behind without `orderFrontRegardless`; App Nap throttles timers…).
...
- **Leia o `AGENTS.md` antes** (notas de manutenção). Ele lista as armadilhas que já morderam este projeto (a trava é global e não está nas seções por fonte; `pmset -g cap` ignora `-b/-c`; janelas abrem atrás sem `orderFrontRegardless`; App Nap freia timers…).
